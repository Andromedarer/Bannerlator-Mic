...
>
> **GPU %/clock (`—`) NOT fixed — device limitation.** kgsl nodes (`gpubusy`/`gpu_busy_percentage`/`devfreq/gpu_load`/`clock_mhz`) exist but are SELinux-labeled `vendor_sysfs_kgsl`; the `untrusted_app` domain is denied, so every candidate path MISSes. Adding paths can't help (same blocked label). Only real options = root-assisted read or accept `—`. GPU **temp** works (thermal zones ARE app-readable). Also noticed **per-core % shows `—`** (per-core clocks work) — separate `/proc/stat` per-cpu gap, not addressed here.
>
