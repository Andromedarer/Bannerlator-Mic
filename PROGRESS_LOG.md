...
## 2026-08-04 — 🩹 **GL/Zink perf-HUD reads 0.0 fps — self-heal fix** (branch `fix/gl-zink-hud-fps-binding`, off main `d1b91464`)

> User: Stronghold Crusader HD under **Zink** shows `0.0 fps` / `1000.0ms` in the Fusion HUD while the game runs fine (GPU 17%, visible on screen). **Root cause:** the FPS tick gate (`XServerDisplayActivity.java:1545`, `frameRatingWindowId == window.id`) is bound via the guest `_MESA_DRV` property, which under Zink rides a render window that is NOT the window actually composited to screen — so the strict match never fires for the presenting window and 0 frames get counted (→ 1000ms/0fps fallback). D3D/DXVK/Vulkan are unaffected (their `_MESA_DRV` window IS the composited one). **Fix:** added a self-heal else-branch — when bound to a `_MESA_DRV` window but the content update comes from a focused top-level app window with no `_MESA_DRV` of its own, re-bind `frameRatingWindowId` to that presenting window (+ add to `mesaDrvWindowIds` so the "upgrade off fallback" logic can't steal it back) and tick. Self-heals once; inert for D3D/Vulkan. Same philosophy as the existing unmap-rebind fallback (~line 6352, "breaks only on OpenGL. Follow the focused application window instead"). Scope = compositor copyArea path (default renderer, the screenshot's path); the 3 `setHudFrameTick` sites left unchanged. ⏭️ CI → stage pubg → device-verify a GL/Zink title reads real FPS (logcat: `GL/Zink HUD self-heal: re-binding FPS…`). See memory `project_bannerlator_gl_zink_hud_fps_binding`.
