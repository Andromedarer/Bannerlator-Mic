...
> **Confidence:** compiles-level (symbols resolve, API floor clear). NOT yet CI-green or device-proven. Device test confirms: (a) plug mid-game → audio follows to headset; (b) unplug mid-game → audio returns to speaker; (c) the 350ms settle window is right on-device (nudge if a beat of silence precedes recovery on unplug).
> ⏭️ NEXT: CI build → stage `pubg` APK → user device-test both directions → merge to main on device-proven.
