...
## 2026-08-09 — 🎧 **Audio follows headphone plug/unplug DURING gameplay** (branch `fix/audio-follow-route-headset` off main `8d432376`, vc 69, NOT on main — awaiting CI + device test)
...
> **Confidence:** recipe is DEVICE-PROVEN live; the C just automates it. Native compile + full APK still need CI + a device pass (does the AudioDeviceCallback-triggered path recover as cleanly as the manual pactl run, both directions).
> ⏭️ NEXT: CI build → stage `pubg` APK → user device-test plug + unplug mid-game → merge on device-proven. Follow-up (optional): also reap the original AAudioSink on first change (needs module enumeration by name).
