...
## 2026-08-10 — 🎮 **#338 phantom on-screen pad shows with profile "Disabled" + fake timeout** (branch `fix/smart-default-touch-disabled-338` off main `ca98c930`, ⏳ CI pending → device test)
...
> **NEXT:** push → CI → stage pubg → user re-runs the exact repro on AYANEO (fresh 2.9.8 container, Disabled, launch) to confirm no phantom pad at launch and no fake timeout. Not merged.
