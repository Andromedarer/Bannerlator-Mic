...
> **Impl:** new `ContainerTopTabs` composable — horizontal, `horizontalScroll` icon+label tab bar, active-tab primary tint + underline, help/reset (`RailLink`s) in a slim right-aligned row above the tabs; long titles abbreviated (ENVIROMENT→ENVIRON, WIN COMPONENTS→WIN COMP). Layout branches on `LocalConfiguration.orientation`: PORTRAIT = `Column`(ContainerTopTabs + content); LANDSCAPE = unchanged `Row`(CollapsibleRail + content). The scrolling content `Box` factored into ONE shared `@Composable () -> Unit` lambda (`mainContent`) reused by both branches (no dup of the `when(activeTab)` dispatch). Collapsed-rail "active tab name over content" header now portrait-dropped (redundant with visible labels), still shown landscape-collapsed. Defaults mode = 4 tabs (no DRIVES). Files: `ui/screens/ContainerDetailScreen.kt` (+imports `horizontalScroll`, `Configuration`). ⏭️ NEXT: CI → stage pubg → device-test (portrait top tabs switch tabs, help/reset work, save ✓ works, landscape rail unchanged, rotate preserves tab/scroll/edits) → merge.
