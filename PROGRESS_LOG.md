...
## 2026-08-01 — 🎞️ **lsfg-vk: adopt GameNative's proven defaults** (branch `feat/lsfg-gamenative-defaults` off main `b7ac9af8`)
...
>
> Compiles-only (CI). NOT device-proven yet. Note: config export/import round-trip does not (yet) enumerate `lsfgAutoEnable`/`lsfgPerformanceMode` — out of scope, flag for later if community-config parity is wanted.
