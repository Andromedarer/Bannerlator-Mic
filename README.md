...
**🎞️ Frame Generation & Present Modes.** Frame-gen was being strangled by FIFO backpressure — it now **forces mailbox while FG multiplies**, and the bundled **win-fg** frame-gen layer is available. A live in-game **Present Mode selector** lands on the Graphics tab (FG gated to the **Vulkan** renderer), with **"?" help + a glossary** across the container/game editors and the shortcut editor. **⚠️ Frame generation is still experimental and a work in progress** — please don't expect magic results; outcomes still vary from game to game, device to device, and with the components used. Both **win-fg** and **lsfg-vk** are offered — treat them as experimental and use whichever works best for your game.
...
**⚠️ Frame generation.** **win-fg** — Bannerlator's clean-room frame-gen layer — is **experimental / a work in progress**; **lsfg-vk** is also offered. Both vary by game and device, so use whichever works best for yours.
